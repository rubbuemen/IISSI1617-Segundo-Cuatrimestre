...
$tutorias = "SELECT OID_T, Fecha, Hora_Comienzo, Duracion, Estado, DNI_Alum FROM Tutorias WHERE DNI_Prof = '".$dni."'";
			...
			<article>
				<div class='contenedor alinearCentro tutorias'>
					<div class='col-100 tablet-col-100 movil-col-100'>
						<h1 class='EncabezadoTutorias'>Tutorías</h1><br />
					</div>
					<div class='col-100 tablet-col-100 movil-col-100'>
						<table class='datosTutoria'>
							...
							<form method='post' action='controladorTutorias.php'>		
				    			<?php 
				    			$i = 0;
				    			foreach($tutoriasPaginada as $tutoria){
									$conexion = crearConexionBD();
									$nombre = nombreAlumno($tutoria['DNI_ALUM'], $conexion);
									cerrarConexionBD($conexion);
						    		...
						    		<tr class='<?php echo $fila; ?>' >
										...
										<?php 	
										if($tutoria['ESTADO']=='Pendiente' || isset($_SESSION['cambiar']) && $_SESSION['cambiar']==$i ){?>
											<td class='confirmar'>
												<button class='confirmarImg' name='confirmar' type='submit' value='<?php echo $tutoria['OID_T']?>'>
													<img class='confirmarImg'src='img/varios/confirmar.png' alt='confirmar' >
												</button>
											</td>
											<td class='denegar'>
												<button class='denegarImg'name='denegar' type='submit' value='<?php echo $tutoria['OID_T']?>'>
													<img class='denegarImg' src='img/varios/denegar.png' alt='denegar'  >
												</button>
											</td>
										<?php	
										} else{ ?>
											<td class='cambiar' colspan='2'>
												<button class='cambiarBoton' name='cambiar' value="<?php echo $i; ?>" type='submit'>
					...
					<section class='col-100 tablet-100 movil-100'>
						...
						for($pagina = 1; $pagina <= $totalPaginas; $pagina++) 
							...
							<?php 
								...
								<a class='enlacePaginacion' href='verTutoriasProfesor.php?PAG_NUM=<?php echo $pagina; ?>&PAG_TAM=<?php echo $paginaTamanio; ?>'><?php echo $pagina; ?></a>
							<?php 
							} ?>
					</section>				
				</div>
			...
			<div class='contenedor alinearCentro'>
				<section class='col-100 tablet-100 movil-100'>
					<button class='cambiarNumPag' id='cambiarNumPag'>Cambiar número de páginas</button>
		   			<form class='cambioPagina' method='GET' action='verTutoriasProfesor.php'>
